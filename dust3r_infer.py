...
def get_args_parser():
    ...
    parser.add_argument("--img_base_path", type=str, default="/home/aogao/code/InstantSplat/data/Tanks_for_dust3r_sim/Church/images")
...
if __name__ == '__main__':
    ...
    images, ori_size = load_images(img_base_path, size=512) 
    print(">> ori_size", ori_size)

    start_time = time.time()
    ##########################################################################################################################################################################################
    pairs = make_pairs(images, scene_graph='consine', prefilter=None, symmetrize=True)
    output = inference(pairs, model, args.device, batch_size=batch_size)
    output_colmap_path = img_base_path.replace("images", "sparse/0")
    if os.path.exists(output_colmap_path):
        print(">> Cleaning existing old output directory")
        shutil.rmtree(output_colmap_path)
