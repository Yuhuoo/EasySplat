...
from simple_knn._C import findKNNCUDA2

def save_pose(path, quat_pose):
    output_poses=[]
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    save_pose(os.path.join(scene.model_path, 'pose', "pose_org.npy"), gaussians.P)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                save_pose(os.path.join(scene.model_path, 'pose', f"pose_{iteration}.npy"), gaussians.P)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration % 2000 == 0:
                    # t1 = time.time()
                    KNN_index = findKNNCUDA2(gaussians.get_xyz)
                    # t2 = time.time()
                    # print('\nknn time(s) : ', f'{t2 - t1:.3f}')
                    # print(KNN_index)
                    gaussians.densify_and_split_KNN(KNN_index)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = ({'name': 'train', 'cameras' : [scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in range(len(scene.getTrainCameras()))]},)
